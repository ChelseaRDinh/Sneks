...
def move():
	...
	# at  this point directions are only valid directios
	direction = best_direction(directions, findFood(snek), snek)
	print(direction)
...
def best_direction(valid_directions, food_place, snake):
		print 'ways to go' 
		if (snake['head']['y'] < food_place[1]):
			if ('down' in valid_directions):
				return 'down'
		if (snake['head']['y'] > food_place[1]):
			if ('up' in valid_directions):
				return 'up'
		if snake['head']['x'] < food_place[0]:
			if ('right' in valid_directions):
				return 'right'
		if snake['head']['x'] > food_place[0]:
			if ('left' in valid_directions):
				return 'left'
		else:
